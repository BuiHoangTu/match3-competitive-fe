...
The packages listed here must be merged into `shell/pubspec.yaml` by the
...
`shell/firebase_options.dart.example` platform checklist).
...
must be added in Xcode. See `shell/firebase_options.dart.example` for the full
...
1. After merging, run `flutter pub get` from `shell/` to resolve the lock file.
2. Run `flutterfire configure` from `shell/` once the Firebase project exists
   (T-v0.6-C01) to generate `shell/lib/firebase_options.dart`.
...
4. Tests under `shell/test/services/` must be invoked with
...
5. `firebase_core` requires `Firebase.initializeApp(options: DefaultFirebaseOptions.currentPlatform)`
   in `shell/lib/main.dart` before any Firebase call. This is an A-track change.
